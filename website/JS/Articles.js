...
window.onload = function () {
    ...
    if(getParameter("id")==""){
        alert("erro url and will go to home page");
    ...
    if(Bmob.User.current()){
        ...
        document.getElementById("welcome").innerHTML =" <a href='login.html?from=2&id="+ getParameter("id")+"'>Login</a>，welcome to my Blog";
    }
    //登录和注销逻辑
    ...
    query.get(getParameter("id"), {
        success: function(gameScore) {
            document.getElementsByTagName("title")[0].innerHTML = gameScore.get("title") + "|Ruisong's personal Website";
            if(Bmob.User.current()){
               if(gameScore.get("requestAuthority")>Bmob.User.current().get("authority")){
                   alert("need account with higher authority");
                   window.open("ArticleList.html","_self");
               }
            }
            else{
            ...
            document.getElementById("bannerThird").innerHTML+="<a href=\"ArticleList.html?dir="+ gameScore.get("type")+"\">"+ gameScore.get("type")+"</a>  >>"+gameScore.get("title");
            //设置分类导航
            ...
            document.getElementById("view").innerHTML =gameScore.get("view") + "views&nbsp;&nbsp;&nbsp;·";//设置观看人数
            document.getElementById("comment").innerHTML = gameScore.get("comments")+" comment";
            //结束 info 逻辑
            //开始正文逻辑 替换pre 标签
    ...
    document.getElementById("uploadComment").onclick = function () {
       ...
       //检查是否空
        ...
        var d2=new Date();
        //获取时间戳
        commenter.trim()==""?commenter="anonymity commenter":commenter;
        if(this.what=="reply"){
    ...
    commentQuery.find({
        success: function(results) {
            ...
            for (var i = 0; i < results.length; i++) {
                ...
                commentDiv.innerHTML = createBy + " said："+comment+"<br/>";
                ...
                if(replies!=undefined){
                    for(var q=0;q<replies.length;q++){
                        commentDiv.innerHTML+= "<br/><p>"+replies[q].name+"  reply： "+ replies[q].content+"</p>";
                    }
                }
                ...
                for(var q=0;q<replyE.length;q++){
                    replyE[q].onclick = function () {
                        ...
                        document.getElementById("uploadComment").value = "upload comment";
                    };
                }
    ...
    typeQuery.find({
        success: function(results) {
            if(results.length==0){
                document.getElementById("typeCata").innerHTML+="no any archive";
            }
            for (var i = 0; i < results.length; i++) {
                var object = results[i];
    ...
    articleQuery.find({
        success: function(results) {
            if(results.length==0){
                document.getElementById("newestArticle").innerHTML+="no recent post";
            }
            for (var i = 0; i < results.length; i++) {
                var object = results[i];
